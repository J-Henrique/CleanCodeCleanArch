...
export default class OrderRepositoryDatabase implements OrderRepository {
	...
	async save(order: Order): Promise<void> {
		...
		for (const orderItem of order.orderItems) {
			await this.connection.query("insert into ccca.order_item (id_order, id_item, price, quantity) values ($1, $2, $3, $4)", [orderData.id_order, orderItem.itemId, orderItem.price, orderItem.quantity]);
		}
	}
